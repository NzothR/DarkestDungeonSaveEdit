...
- 负面怪癖替换执行“删除旧怪癖，再增加新怪癖”；怪癖顺序没有 UI 意义，不要求在原位置增加，负面怪癖不带替换标志。
...
建筑集中显示在左侧名单中。点击建筑打开升级窗口，显示该建筑原版升级文件中的各条升级链；建筑升级暂按用户确认的原版建筑树实现，不从 Mod 加载新增建筑或升级树。名单顶部提供图标式“全部建筑升满”操作，建筑窗口提供当前建筑全部升满操作。

英雄技能与装备升级控件沿用 v1.0 交互：左键 +1、右键 -1、Shift+左键最大、Shift+右键最小；同时提供键盘和明确的可访问按钮。右键语义只对配置了升级档案的控件生效，不全局套用。具体最大等级来自当前有效升级树，不硬编码原版上限。

升级窗口中，左键点击节点会升级到该节点并自动解锁前置节点；右键点击节点会降到该节点之前并锁定后续节点。节点悬浮提示读取原版 `str_{treeId}_upgrade_lvl_{level}` 本地化说明；最大等级由对应原版升级树提供。
