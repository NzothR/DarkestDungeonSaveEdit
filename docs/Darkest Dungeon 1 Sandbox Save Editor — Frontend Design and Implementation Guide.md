...
- 已有完整配置时直接进入数据库初始化，不要求用户重新填写设置；初始化完成后点击完成进入临时 Mod 列表页。
- 初始化运行期间，保存按钮变为灰色不可用的状态按钮；任务完成后同一个位置变为“完成”，点击后才显示 Mod 列表。
- Mod 列表从 `mod_environment.db` 查询，按存档顺序显示数据库本地化名称；缺失 Mod 保留存档 key/name，并显示诊断。
- 如果 Mod 目录中存在 `preview.*`、`cover.*` 或 `mod_preview.*`，列表最左侧显示封面图；封面缺失时保留文本条目。
