...
- 负面怪癖替换执行“删除旧怪癖，再在原位置增加新怪癖”；负面怪癖不带替换标志。正面怪癖也须在原位置替换。
...
游戏目录和备份目录必须填写；存档 Profile、Workshop Mod 和额外本地 Mod 可以留空，后续设置中再补充。
所有目录各只有一个。页面使用右侧的目录选择按钮调用本地原生目录选择器，输入框只读，避免让用户手写路径。

存档选择器直接选择单个 `profile_*` 目录。后端立即检查核心存档结构，验证失败时不接受该路径。

界面语言和游戏内容本地化分开。界面资源位于 `web/assets/locales/`，首版提供 `en_us.json` 和 `zh_cn.json`；
设置下拉框默认选择简体中文。用户改变下拉框后立即切换页面资源，不需要先点击保存；保存按钮只负责持久化选择。
后续新增语言只需增加资源文件并注册 locale。
...
八栋可升级建筑集中显示在左侧名单中；墓园没有升级树，不列入名单。点击建筑打开升级窗口，显示当前有效内容环境中该建筑升级文件的各条升级链，包括启用 Mod 对原版文件的覆盖；不从 Mod 加载新增建筑。名单顶部提供图标式“全部建筑升满”操作，建筑窗口提供当前建筑全部升满操作。

英雄技能与装备升级控件沿用 v1.0 交互：左键 +1、右键 -1、Shift+左键最大、Shift+右键最小；同时提供键盘和明确的可访问按钮。右键语义只对配置了升级档案的控件生效，不全局套用。具体最大等级来自当前有效升级树，不硬编码原版上限。

升级窗口中，左键点击节点会升级到该节点并自动解锁前置节点；右键点击节点会降到该节点之前并锁定后续节点。节点悬浮提示读取 `str_{treeId}_upgrade_lvl_{level}` 本地化说明；最大等级由当前有效升级树提供。

“小镇建筑系统开放”“单个区域建筑建成/锁定”“普通建筑升级进度”是不同状态，分别呈现。点击“区域建筑”进入独立窗口，可左键解锁/右键锁定单栋区域建筑，并提供批量解锁、批量锁定并关闭区域建筑功能，以及单独切换区域建筑功能的操作。
...
给英雄添加饰品时，操作请求必须包括目标 Hero ID 和具体饰品箱条目键或内容定义 ID。前端自动按当前英雄职业筛除不适用项，后端再次验证。已有饰品左键单击替换、拖动调整位置；右键放回饰品箱，Shift+右键直接销毁。拖动和删除均应有立即的操作结果和撤销入口。
...
- 配置安装目录、可选 Workshop Mod、可选额外本地 Mod、单个存档 Profile 和语言。
- 配置并初始化独立备份目录、普通备份数量上限和自动编辑保存策略。
- 启动时检查 `AutoEditSave` 恢复点，并提供恢复/放弃入口。
- 保存配置后进入数据库初始化任务，显示原版数据库、Mod 环境数据库和总耗时、当前工作与进度。
- 已有完整配置时检查数据库结构；数据库完整则直接复用并进入小镇主界面，不要求用户重新填写设置或等待重复扫描。数据库缺失/损坏时才显示初始化任务，完成后点击“完成初始化”进入小镇主界面。
- 初始化运行期间，保存按钮变为灰色不可用的状态按钮；任务完成后同一个位置变为“完成初始化”，点击后才显示小镇主界面。
- Mod 列表从 `mod_environment.db` 查询，按存档顺序显示数据库本地化名称；缺失 Mod 保留存档 key/name，并显示诊断。该列表暂不作为初始化完成后的主流程页面。
- 设置区域提供“重新初始化 Mod 数据库”按钮，用户确认 Mod 环境发生变化后主动触发更新。
- Mod 图标从 Mod 根目录的固定文件 `preview_icon.png` 读取，列表最左侧显示；封面缺失时保留文本条目。

## F2 — Town Shell 与基础导航

- 构建原版小镇视觉、资源区、建筑名单、英雄名单、饰品箱和保存状态栏。
- 第一轮先交付 Town Shell，初始化完成后点击“完成初始化”进入该界面；数据区由 Campaign Model 驱动，建筑仍保持只读。
- 小镇场景使用 1920×1080 的归一化坐标布局；背景优先使用游戏目录中的 `fx/town_ground/town_ground.sprite.png`，后端通过本地游戏资源接口读取背景和建筑图片，找不到资源时显示文字白板占位。
- 建筑以集中名单显示，只展示建筑名称和“区域建筑”分组文字；具体升级/降级控件留到各建筑自己的编辑界面。建筑不再以分散的场景热点作为主要操作入口。资源位于左下角，建筑名单位于左侧，英雄名单位于右侧，饰品箱位于中央，设置入口位于右下角。
- 先完成导航、状态、占位内容、工具提示及侧栏布局，再逐项接入编辑命令。
- 资源数值可直接输入；资源修改进入后端 Session，要求携带 revision。
...
- 游戏本地化语言同时兼容 `schinese` 与 `chinese` 两种 Mod 常见语言标识，并清理 `[colour_start]`、`[C2]` 以及编译后 `.loc2` 中的 `<c>XXXXXX...</c>` 颜色标记后再显示；其中 `XXXXXX` 是游戏颜色编码，不属于文本内容。
...
- 建筑名单、当前有效升级树和区域建筑状态已接入 Campaign Session；升级节点经后端 Operation/Validation 修改，并进入相同的 Undo/Redo 与显式 Save 流程。
- 饰品箱提供单件添加、按 Mod/职业筛选的批量添加与批量删除；批量添加默认允许重复饰品，可选只添加库存中尚不存在的 ID。批量操作在后端以单个映射事务进入 Undo/Redo。
- 饰品搜索优先级为原始 ID、本地化键、当前语言名称、英文名称、效果、Mod 来源、标签；采用 Unicode NFKC 与大小写/空白归一化，不按英文空格规则切分中文。简单正则使用 `/pattern/flags` 写法，例如 `/bleed/i`。
- 饰品条目支持左键拖动重排、右键删除；悬浮卡片展示游戏本地化名称、职业限制、稀有度、由可见 buff 生成的本地化效果，以及 Mod 本地化名称和来源唯一键。`has_description=false` 的实现用隐藏 buff 不进入详情；没有本地化键的未知效果不猜测文本。选择器条目共用同一详情卡片；悬浮卡片使用浏览器顶层 popover，避免被选择器遮住。效果行居中，长详情可在悬停饰品时按 Shift+滚轮滚动。后端在 CampaignSession 初始化时预热完整目录及效果数据，弹窗打开后只用内存数据。

## F4 — Operation History 与安全保存

- 扩展首轮资源操作之外的批量操作历史，补齐校验、预览、显式 Commit、备份恢复和失败诊断。
- 首轮资源编辑已经提供即时 Undo/Redo、Session dirty 状态和 revision 冲突处理；F4 负责把这些状态接入完整保存事务。
