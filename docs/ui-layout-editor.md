...
这是一个独立运行的布局制作工具，不属于主前端页面。它只扫描已经处理好的图片，保存图片与 UI 关联之间的关系以及 1920×1080 画布上的位置和尺寸。
...
1. 点击顶部“添加组件”。
2. 搜索并选择一张图片。
3. 选择预设关联，或点击“新增关联”填写名称和 ID。
4. 普通组件会放在画布中央并立即进入编辑模式。拖动组件改变位置，拖动右下角控制点按比例缩放。
5. 点击“编辑完成”退出编辑模式。之后右键组件可以再次编辑。
6. 点击底部“保存”，输入文件名后写入布局 JSON。文件名只能是文件名本身，扩展名不是 `.json` 时会自动补上；每次保存可以写入同一输出目录下的不同布局文件。
...
画布、组件、关联和几何数据不包含小镇专用字段。当前小镇名称列表位于 `tools/ui_layout_editor/presets.json`，也可以通过 `--preset-file` 换成其他界面的预设关联：

```powershell
python tools/ui_layout_editor.py --preset-file .\config\hero-editor-presets.json
```

因此后续制作英雄编辑界面时可以复用同一个编辑器，只需要提供新的关联预设和美术资源目录。`free` 关联允许拖动缩放，`background` 与 `bottom_bar` 是目前定义的固定放置类型；其他特殊放置规则可以继续扩展为新的 placement 类型。
