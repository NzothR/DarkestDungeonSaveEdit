...
- 正面和负面怪癖上限分别在第一次读取存档时取所有英雄中对应极性的最大数量，写入配置，之后不重新计算。调低上限时，逐个英雄只从对应极性列表末尾删除超出记录。界面不显示全部空槽，仅在尚未达到上限时显示一个末尾加号；替换须保留原列表位置。
- 怪癖选择器只列出点击区域对应极性的有效非疾病定义，禁止同一英雄重复 ID。正面替换保留原记录并在原位重命名键；负面替换删除旧记录后在原位创建新记录，不保留替换元数据。新增使用内置稳定字段模板，空怪癖英雄也可新增。右键删除须确认，Shift+右键直接删除。仅 `can_modify_in_activity` 的正面怪癖显示锁定/解锁按钮。怪癖效果在英雄列表和选择器中悬浮显示。上述流程仍需游戏内验收。
...
| `campaign.hero.add_or_replace_quirk` | 英雄怪癖集合 | 新增时使用稳定记录模板；正面替换保留原记录并原位改键，负面替换删除旧记录后在原位创建新记录 |
