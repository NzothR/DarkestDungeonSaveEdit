...
- 英雄饰品槽上限首次打开存档时取当前英雄中最大的装备数量，写入 `config.json` 的 `heroTrinketSlotLimit`；后续加载不重新计算。调低上限时，对每名英雄从末尾删除超出的完整饰品记录；空槽只存在于界面，不向 DSON 写占位对象。
- 装备弹窗默认从饰品箱选择具体库存条目，取出时只删除该条目的原始键，保留其余库存顺序；也可从内容数据库创建。默认拒绝同一英雄装备重复 ID，并校验有效定义的职业限制。
- 左键单击已装备饰品会替换：删除原完整记录，并在原键创建新记录。拖动以数字键重排英雄饰品；右键放回饰品箱末尾，Shift+右键销毁。饰品箱与英雄之间转移时保留物品类型、数量、附加效果和转化状态字段。悬浮详情沿用饰品箱字段。以上 UI/API 已用隔离存档副本验证保存重载，仍需游戏内验收。
- 英雄饰品新增/替换弹窗按当前英雄的职业自动筛选，隐藏多余的职业筛选控件；饰品箱的批量弹窗仍提供职业筛选。
- 正面和负面怪癖上限分别在第一次读取存档时取所有英雄中对应极性的最大数量，写入配置，之后不重新计算。调低上限时，逐个英雄只从对应极性列表末尾删除超出记录。怪癖没有有序槽位，界面仅在尚未达到上限时显示一个末尾加号。
- 怪癖选择器只列出点击区域对应极性的有效非疾病定义，禁止同一英雄重复 ID。正面替换保留原记录并重命名键；负面替换删除旧记录再创建新记录，不保留替换元数据，也不要求保持原顺序。新增使用内置稳定字段模板，空怪癖英雄也可新增。右键删除须确认，Shift+右键直接删除。仅 `can_modify_in_activity` 的正面怪癖显示锁定/解锁按钮。怪癖效果在英雄列表和选择器中悬浮显示。上述流程已用隔离存档验证保存重载，仍需游戏内验收。
...
| `campaign.hero.add_or_replace_quirk` | 英雄怪癖集合 | 新增时使用稳定记录模板；正面替换保留原记录并改键，负面替换删除旧记录后创建新记录，不要求保持原顺序 |
