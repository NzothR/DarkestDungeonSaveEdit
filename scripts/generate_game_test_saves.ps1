param(
    ...
    [string]$OutputRoot = "test_save_profile\stage10_game_tests_verified"
)
...
if (Test-Path -LiteralPath $cachePath -PathType Leaf) {
    ...
    if ($cacheValues.ContainsKey('CMAKE_COMMAND')) {
        $configuredCmake = $cacheValues['CMAKE_COMMAND']
        if (Test-Path -LiteralPath $configuredCmake -PathType Leaf) { $cmake = $configuredCmake }
    }
}
if (-not $cmake) {
...
# CMake may link against DLLs supplied by its bundled MinGW runtime and SQLite package.
# Add only those build-time dependency directories to this script process.
$runtimeDirectories = @()
if ($cacheValues.ContainsKey('CMAKE_CXX_COMPILER')) {
    $runtimeDirectories += Split-Path -Parent $cacheValues['CMAKE_CXX_COMPILER']
}
...
Write-Host "Building ddse_generate_game_test_saves..."
& $cmake --build $buildPath --target ddse_generate_game_test_saves --config Debug
if ($LASTEXITCODE -ne 0) {
    throw "CMake build failed with exit code $LASTEXITCODE"
}

$candidateExecutables = @(
    (Join-Path $buildPath "ddse_generate_game_test_saves.exe"),
    (Join-Path $buildPath "Debug\ddse_generate_game_test_saves.exe"),
    (Join-Path $buildPath "ddse_generate_game_test_saves")
)
$generator = $candidateExecutables | Where-Object { Test-Path -LiteralPath $_ -PathType Leaf } | Select-Object -First 1
if (-not $generator) {
    throw "Built generator executable was not found under $buildPath"
}

Write-Host "Generating isolated test profiles from $sourcePath ..."
& $generator `
...
if ($LASTEXITCODE -ne 0) {
    throw "Save generation failed with exit code $LASTEXITCODE"
}
