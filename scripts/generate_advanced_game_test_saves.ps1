param(
    ...
    [string]$OutputRoot = "test_save_profile\stage11_advanced_game_tests"
)
...
Write-Host "Building advanced test save generator..."
& $cmake --build $buildPath --target ddse_generate_advanced_game_test_saves --config Debug
if ($LASTEXITCODE -ne 0) { throw "CMake build failed with exit code $LASTEXITCODE" }

$candidateExecutables = @(
    (Join-Path $buildPath "ddse_generate_advanced_game_test_saves.exe"),
    (Join-Path $buildPath "Debug\ddse_generate_advanced_game_test_saves.exe"),
    (Join-Path $buildPath "ddse_generate_advanced_game_test_saves")
)
